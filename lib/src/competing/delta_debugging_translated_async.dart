...
//        return self.coerce(sorted_c)
  ...
  Future<Result> test(List<Delta> c) async {
    ...
    if (cache_outcomes != null) {
      ...
      if (cached_result != null) {
        return Future.value(cached_result);
      }
    }
    ...
    if (monotony) {
      ...
      if (cached_result == Result.pass) {
        return Future.value(Result.pass);
      }
...
//    print "test(" + self.coerce(c) + ")..."
  ...
  Future<Result> doTest(List<Delta> c);
  ...
  List<Delta> resolve(csub, c, direction) {
...
//		  `direction` + ") = " + `outcome`
  ...
  /// Stub to overload in subclasses.
  List<Delta> _resolve(csub, c, Direction direction) {
  ...
  Future<Pair<Result, List<Delta>>> test_and_resolve(List<Delta> csub,
      List<Delta> r, List<Delta> c, Direction direction) async {
    final initial_csub = List<Delta>.from(csub);
    final c2 = __listunion(r, c);

    var csubr = __listunion(csub, r);
    var t = await test(csubr);
...
//	    run = run + 1
  ...
  Future<Pair<Result, List<Delta>>> test_mix(
      List<Delta> csub, List<Delta> c, Direction direction) async {
    if (minimize) {
      var pair = await test_and_resolve(csub, <Delta>[], c, direction);
      var t = pair.a;
      csub = pair.b;
      if (t == Result.fail) {
        return pair;
      ...
      if (maximize) {
        ...
        final pair =
            await test_and_resolve(csubbar, <Delta>[], cbar, directionbar);
        final tbar = pair.a;
        csubbar = pair.b;
  ...
  Future<List<Delta>> ddgen(List<Delta> c, bool minimize, bool maximize) {
...
//	    print ("dd(" + self.pretty(c) + ", " + `n` + ") = " + `outcome`)
  ...
  /// Stub to overload in subclasses
  Future<List<Delta>> _dd(List<Delta> c, int n) async {
...
//	    print
      ...
      for (int i = 0; i < n; ++i) {
        ...
        var pair = await test_mix(cs[i], c, Direction.remove);
        final t = pair.a;
        cs[i] = pair.b;
      ...
      if (!c_failed) {
        ...
        for (var j = 0; j < n; ++j) {
          ...
          var pair = await test_mix(cbars[i], c, Direction.add);
          final t = pair.a;
          cbars[i] = pair.b;
  ...
  Future<List<Delta>> ddmin(List<Delta> c) => ddgen(c, true, false);
